...
when a rule is entirely matched. The userspace tool \code{iptables} is
used to manipulate the IP tables. The data structures are designed
such that they can be used in both user and kernel mode.
...
looked up everytime it arrives on an interface or sent from an
...
One example of space saving was the use of
...
\verb|list_head| is not needed. The \verb|hlist_head| contains only a
